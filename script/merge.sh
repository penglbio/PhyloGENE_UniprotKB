...
# 临时存储合并结果的文件
temp_file1="temp_merged1_all.csv"
temp_file2="temp_merged2_all.csv"

# 合并前两个文件作为初始结果
csvtk join -tHl -f "1,2;1,2" --outer-join --na NA "${tsv_files[0]}" "${tsv_files[1]}" > "$temp_file1"

# 从第二个文件开始，逐个与临时文件合并
for (( i=2; i<${#tsv_files[@]}; i++ )); do
    csvtk join -tHl -f "1,2;1,2" --outer-join --na NA "$temp_file1" "${tsv_files[$i]}" > "$temp_file2"
...
# 假设每个文件名代表一个不同的列
header=""
for file in "${tsv_files[@]:0:$num_args}"; do
...
#输出header到merge文件
echo -e "taxid\tlineage\t$header" > $output_file
# 最后，将临时文件重命名为最终输出文件
